...
@pytest.mark.asyncio
async def test_multiple_streams_mixed_read_order():
    # Node A should be able to open a stream with node B and then vice versa.
    # Stream IDs should be generated uniquely so that the stream state is not overwritten
    node_a = await new_node(transport_opt=["/ip4/127.0.0.1/tcp/0"])
    node_b = await new_node(transport_opt=["/ip4/127.0.0.1/tcp/0"])
    ...
    node_a.set_stream_handler("/echo_a/1.0.0", stream_handler_a)
    node_a.set_stream_handler("/echo_b/1.0.0", stream_handler_b)
    node_b.set_stream_handler("/echo_a/1.0.0", stream_handler_a)
    node_b.set_stream_handler("/echo_b/1.0.0", stream_handler_b)
    ...
    print("REACHES ME0")
    stream_a = await node_a.new_stream(node_b.get_id(), ["/echo_a/1.0.0"])
    print("REACHES ME1")
    stream_b = await node_a.new_stream(node_b.get_id(), ["/echo_b/1.0.0"])
    print("REACHES ME2")
    # A writes to /echo_b via stream_a, and B writes to /echo_a via stream_b
    messages = ["hello" + str(x) for x in range(10)]
    for message in messages:
        print("test_multiple_streams_mixed_read_order sending messages")
        a_message = message + "_a"
        b_message = message + "_b"
